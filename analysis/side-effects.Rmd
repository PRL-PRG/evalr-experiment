...
Fix known missing eval source code references.
...
Remove the side effects that happen to `.Random.seed` unless it comes from the actual call to `set.seed`.
...
Remove side effects from R internal use of eval.
...
Normalize callers based on the synthesized eval call site source reference.
...
The following computes the overall information about side effects done in eval calls.
...
The following classifies the side effects:
...
### Target environments for side-effects <a name="table_se_target_envs" />

```{r}
table_se_target_envs <- 
  left_join(
...
```{r}
datatable(table_se_target_envs)
```
...
### Types of eval side-effects <a name="table_se_types" />
